...
## 🌟 Các Năng Lực Cốt Lõi (Theo Pitch Deck W6)
...
* **🗺️ Map Intelligence**: Hỗ trợ tra cứu bản đồ, chỉ đường trực quan (OSRM), gợi ý tuyến đi tối ưu và mô phỏng mật độ giao thông, tài xế theo thời gian thực.
...
> [!IMPORTANT]
> ### 💎 Cập Nhật Mới Tại Phase 10 (Map Intelligence & OSRM Routing)
> * **Tool-calling ReAct Agent**: Tích hợp luồng Map Intelligence chuyên biệt cho các câu hỏi về bản đồ. AI chủ động gọi các công cụ (tools) qua OpenAI Function Calling để định vị địa điểm (Nominatim OSM), lấy dữ liệu dẫn đường (OSRM) và truy vấn dữ liệu kẹt xe.
> * **Xử lý thiếu vị trí thông minh**: Khi không có dữ liệu tọa độ từ Frontend hoặc Context, hệ thống sẽ ngừng suy luận và tự động gửi UI Form để người dùng bổ sung vị trí, thay vì "đoán mò".
> * **Render bản đồ trực quan**: Kết hợp với React-Leaflet để render bản đồ ngay trong giao diện chat, hỗ trợ đa tuyến đường (mỗi tuyến một màu riêng biệt), kèm theo các điểm xuất phát/đích rõ ràng.
>
> [!IMPORTANT]
...
graph TD
    ...
    E -- "small-talk / sensitive / missing_info" --> NLU_Direct[Return NLU suggested_answer / Ask clarification]
    E -- "rag" --> F{Second Exact Cache}
    E -- "food_recommendation" --> M{Missing info?}
    E -- "map_intelligence" --> MAP1{Missing location?}

    %% Luồng Map Intelligence
    MAP1 -- yes --> FORM
    MAP1 -- no --> MAP2[Tool: search_places & get_osrm_routes]
    MAP2 --> MAP3[Tool: get_traffic_zones]
    MAP3 --> MAP4[Map Answer LLM Synthesis]
    MAP4 --> MAPCARD[Map Cards UI + SSE Stream]
    MAPCARD --> Out
    ...
    style NLU_Direct fill:#f59e0b,stroke:#fff,color:#fff
    ...
    style MAP2 fill:#3b82f6,stroke:#fff,color:#fff
    style MAP3 fill:#3b82f6,stroke:#fff,color:#fff
    style MAP4 fill:#3b82f6,stroke:#fff,color:#fff
    style MAPCARD fill:#3b82f6,stroke:#fff,color:#fff
...
* Chi tiết luồng xử lý Map Intelligence và Agent Tool-calling xem tại: [`docs/MAP_PIPELINE.md`](docs/MAP_PIPELINE.md)
