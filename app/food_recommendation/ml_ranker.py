...
class XGBoostFoodRanker(BaseFoodRanker):
    ...
    def __init__(self, model_path: str | None = None):
        ...
        # Khi có model, sẽ load xgb.Booster(model_file=model_path)
    ...
    def rank(
        ...
        if not self.is_loaded:
            # Pass-through nếu chưa có model
            return sorted(
                [(candidate_scores.get(c.item_id, 0.0), c) for c in candidates],
                key=lambda x: x[0],
                reverse=True
            )
        # TODO: Trích xuất features (User profile, Context, Item features) -> predict -> return
        pass
