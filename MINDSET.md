...
> 2. Chặn lời chào / cảm ơn để tránh gọi LLM thừa.
> 3. Kiểm tra cache trả lời cũ nếu có.
...
graph TD
    %% Nodes
    Q[❓ 1. Nhận Câu Hỏi Người Dùng] --> Intercept[🛡️ 2. Greeting / Cache Check]
    Intercept --> QR[🧠 3. Query Rewrite]
    QR --> QE[🔧 4. Query Expansion]
    QE --> SG[👤 5. Role Filtering / Shared Search]
    ...
    F_Cust & F_Driv & F_Merc & F_Agent --> Dense[🔍 6. Dense Semantic Search<br>ChromaDB Embedding Matching]
    F_Cust & F_Driv & F_Merc & F_Agent --> Sparse[🔍 6. Sparse Keyword Search<br>BM25 Inverse Indexer]

    Dense --> RRF[⚡ 7. Reciprocal Rank Fusion - RRF]
    Sparse --> RRF

    RRF -->|Top 30 Chunks| Rerank[🚀 8. Cross-Encoder Reranker<br>Tính toán mức liên quan siêu tốc]
    Rerank -->|Top 5 Chunks| Compress[✂️ 9. Parent-Child Context Compression]
    Compress --> LLM[🤖 10. LLM Response Generation<br>Strict System Prompt Enforcement]
    LLM --> Citation[🛡️ 11. Citation & Source Validator]

    Citation -->|Hoàn Tất Phản Hồi| Answer[🎉 12. Câu trả lời sạch + Nguồn trích dẫn chính thức]
