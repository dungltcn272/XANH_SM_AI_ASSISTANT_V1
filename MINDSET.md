...
> 11. **LLM Generator & Faithfulness Check**: LLM tổng hợp phản hồi và chạy module kiểm toán chống ảo giác (Faithfulness Check). Nếu điểm quá thấp, kích hoạt Fallback thô chính xác để bảo vệ uy tín.
> 12. **Citation Validator**: Xác thực nguồn sạch, đối khớp URL và trả phản hồi cuối cùng.
...
graph TD
    ...
    ParentChild --> LLM[🤖 11. LLM Generation<br>Synthesizer]
    LLM --> Faith{🛡️ 12. Faithfulness Check<br>Hallucination Evaluator}
    
    Faith -->|Thấp < 0.6| Fallback[🛠️ Strict Offline Fallback<br>Hiển thị trích dẫn thô]
    Faith -->|Đạt chuẩn| Citation[📖 13. Citation & Response<br>Nguồn trích dẫn sạch]
