...
async def listen_messages(websocket, drivers, send_queue):
    driver_dict = {d.driver_id: d for d in drivers}
    try:
        while True:
            message = await websocket.recv()
            data = json.loads(message)
            if data.get("type") == "ride_offer":
                bid = data["booking_id"]
                did = data["driver_id"]
                p_lat, p_lng = data["pickup_lat"], data["pickup_lng"]
                d_lat, d_lng = data["dropoff_lat"], data["dropoff_lng"]
                
                print(f"Driver {did} received offer for {bid}")
                if did in driver_dict:
                    driver_dict[did].status = "busy"
                    asyncio.create_task(process_offer(send_queue, driver_dict[did], bid, p_lat, p_lng, d_lat, d_lng))
                else:
                    print(f"Warning: Driver {did} not found in this simulator instance!")
            elif data.get("type") == "cancel_offer":
                did = data["driver_id"]
                if did in driver_dict:
                    driver_dict[did].status = "available"
                    print(f"Driver {did} received cancel_offer, reverting to available.")
...
async def ws_sender(websocket, send_queue):
    try:
        while True:
...
async def main():
    print("Starting simulator with OSRM Routing...")
    while True:
        try:
            async with websockets.connect("ws://localhost:8000/ws/simulator") as websocket:
                ...
                tasks.append(asyncio.create_task(listen_messages(websocket, drivers, send_queue)))
                tasks.append(asyncio.create_task(ws_sender(websocket, send_queue)))
