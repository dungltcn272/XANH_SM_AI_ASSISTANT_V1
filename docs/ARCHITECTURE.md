...
Tầng hỏi đáp tri thức Xanh SM (Đã phân bổ vào các thư mục chức năng cụ thể):

- `core/`: Chứa các thành phần cốt lõi điều phối:
  ...
  - `pipeline.py`: wrapper/eval pipeline cũ.
- `search/`: Chứa các thuật toán truy xuất và mở rộng câu hỏi:
  - `hybrid_search.py`: tìm kiếm dense/sparse kết hợp boost metadata.
  - `multi_query.py`: mở rộng query.
  - `reranker.py`: xếp hạng lại (Cohere Rerank).
  - `domain_vocabulary.py`: từ vựng domain, mapping lỗi chính tả.
- `storage/`: Chứa các bộ nhớ đệm và lưu vết:
  - `cache.py`: semantic cache.
  - `trace_store.py`: lưu vết log truy xuất RAG.
- `security/`: Bảo mật và kiểm duyệt nội dung đầu ra:
  - `guardrail.py`: guardrail output/bảo vệ kết quả.
- `compat/`: Đảm bảo tính tương thích:
...
Tầng gợi ý món ăn/quán ăn (Đã phân bổ vào các thư mục chức năng cụ thể):

- `core/`: Điều phối và cấu trúc dữ liệu cốt lõi:
  - `chain.py`: orchestrator gợi ý món ăn.
  - `tool.py`: public tool `recommend_food()`.
  - `schemas.py`: request/response/dataclass/schema.
  - `nlu.py`: chuyển đổi NLU slots thành food slots.
- `search/`: Tìm kiếm ứng viên:
  - `retrieval.py`: tìm kiếm ứng viên theo danh mục/khoảng cách/khẩu vị.
  - `catalog.py`: nạp/chuẩn hóa food catalog.
- `ranking/`: Xếp hạng ứng viên:
  - `ranker.py`: bộ xếp hạng quán/món theo khoảng cách, ETA, rating.
  - `ml_ranker.py`: bộ xếp hạng thử nghiệm ML (XGBoost / Bandit).
  - `features.py`: trích xuất đặc trưng cho Ranker.
- `location/`: Xử lý vị trí địa lý:
  - `location_resolver.py`: giải quyết vị trí hiện tại/đã lưu của user.
  - `geocode.py`: đổi địa chỉ sang tọa độ.
- `generation/`: Tạo phản hồi tự nhiên:
  - `answer_llm.py`: gọi LLM sinh gợi ý và chèn card marker.
  - `payloads.py`: xây dựng payload phản hồi và cấu trúc card.
- `profile/`: Cá nhân hóa và hồ sơ khẩu vị:
  - `profile.py`: chuẩn hóa văn bản, hỗ trợ tạo profile.
  - `profile_store.py`: đọc/ghi hồ sơ khẩu vị và sở thích.
- `storage/`: Lưu trữ nhật ký:
  - `trace_store.py`: ghi vết chi tiết thuật toán xếp hạng và phản hồi.

Quy tắc card: BE stream chữ + marker `[[FOOD_CARD {...}]]`; FE chịu trách nhiệm ẩn marker đang dở, hiện shimmer, và render card inline.
