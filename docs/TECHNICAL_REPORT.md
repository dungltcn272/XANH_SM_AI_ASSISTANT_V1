...
* **SQL Tables & Geospatial:** Hệ thống cơ sở dữ liệu nhà hàng với hàng chục nghìn điểm bán (POI) chứa thông tin tọa độ (Lat/Lng), khoảng giá, menu, và đánh giá.
...
Kiểm thử được chạy trên tập dữ liệu tiêu chuẩn `golden_50` chứa các kịch bản thực tế khó nhất từ bộ phận vận hành Xanh SM. Các bộ chỉ số (Metrics) dùng để đo lường bao gồm:
